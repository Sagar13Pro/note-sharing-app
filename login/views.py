...
def Dashboard(request):
    if request.session.get('session_email'):
        note = list(Note.objects.filter(creator_id=Users.objects.get(
            email=request.session.get('session_email'))).values())
        for item in note:
        ...
        if Notes.expiry_time > datetime.now():
            if len(request.POST) > 0:
                ...
                if Note.set_PasswordDecrypt(Notes.password) == request.POST["password"]:
                    ...
                    messages.error(
                        request, "It seems you entered incorrect password.")
        ...
        try:
            ...
            if note_valid.is_valid():
                Note.objects.create(
                    ...
                    secured_content=Note.set_PasswordEncrypt(
                        note_valid.data['secured_content']),
                    password=Note.set_PasswordEncrypt(
                        note_valid.data['password']),
                    isset_Password=note_valid.data['isset_Password'],
                    creator=user)
                messages.success(
                    request, "Note successfully created with title: %s" % note_valid.data['title'])
            else:
...
def EditStore(request, id=None):
    if request.method == 'POST':
        ...
        if contents.is_valid():
            ...
            to_update.secured_content = Note.set_PasswordEncrypt(
                contents.data['secured_content'])
            if contents.data['isset_Password'] == 'False':
                ...
                if contents.data['password'].startswith('gAAAAA'):
                    ...
                    to_update.password = Note.set_PasswordEncrypt(
                        contents.data['password'])
                    to_update.isset_Password = contents.data['isset_Password']
            to_update.save()
            messages.success(
                request, 'Hurray!! Your Note with id %d has been successfully updated.!!' % id)
...
def ShareNote(request, id=None):
    if request.method == 'POST':
        try:
            ...
            send_mail(
                'Note Share Link',
                'Live Note Shared link: ' + noteLink,
                settings.EMAIL_HOST_USER,
                [request.POST['emailto']]
            )
            messages.success(
                request, 'Your note has been shared to %s successfully. Also check in spam for mail' % request.POST['emailto'])
            return HttpResponseRedirect(reverse('user_dashboard'))
        except Exception as error:
            print(error)
            messages.error(request, 'OPPS! Something went Wrong!!')
...
def Delete(request, id=None):
    ...
    if notes.expiry_time is not None:
        if notes.expiry_time > datetime.now():
            messages.info(
                request, "The Note is not expired.You can't Delete Now.")
            return HttpResponseRedirect(reverse('user_dashboard'))
        else:
            notes.delete()
            messages.success(
                request, "Your Note with id %d is Deleted Successfully!" % id)
            return HttpResponseRedirect(reverse('user_dashboard'))
    else:
        notes.delete()
        messages.success(
...
def search_Result(request):
    ...
    if len(answer_notes) == 0:
        try:
            ...
            result = list(Note.objects.filter(creator=user_id))
            for i in result:
...
def reset_done(request):
    if request.POST['new_pass'] == request.POST['confirm_pass']:
        ...
        tmp.password = kdf.hash(
            request.POST['new_pass'], rounds=1200, salt_size=32)
...
def validate_password(request):
    try:
        ...
        password_Reset_link = "https://localhost:8000/user/reset_password/" + \
            str(exist_user.id)
